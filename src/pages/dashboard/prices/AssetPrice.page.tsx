...
type DispatchProps = {
    type: 'assetInfoHasError' | 'assetSummaryHasError' | 'assetChartHasError' | 'assetMarketsHasError' | 'assetInfoHasNoError' | 'assetSummaryHasNoError' | 'assetChartHasNoError' | 'assetMarketsHasNoError'
}
...
const ErrorContainer = styled.div.attrs<{ $hasError: boolean }>(({$hasError}) => ({
...
export function Component(): JSX.Element {
    ...
    const hasError = assetInfoHasError ?? assetSummaryHasError ?? assetChartHasError ?? assetMarketsHasError
        ...
        <>
            {
                ...
                <ErrorContainer $hasError={hasError}>
                    <ErrorWrapper>
                        <Heading headingType={'h3'}>Sorry, There is an error!</Heading>
                        <span className={'error-message'}>
                                There was an error while fetching asset data from the server, Please refetch or contact with the admin.
                            </span>
                        <Button onClickHandler={() => setRefetchListener(refetchListener + 1)}>
                            Try again
                        </Button>
                    </ErrorWrapper>
                </ErrorContainer>
            ...
            <AssetPriceContainer $hasError={hasError}>
                <AssetPriceWrapper>
                    ...
                    <AssetInfo assetName={assetName as AssetName}
                               ...
                               hasErrorHandler={() => dispatchError({type: 'assetInfoHasError'})}
                               hasNoErrorHandler={() => dispatchError({type: 'assetInfoHasNoError'})}
                    />
                    <AssetSummary assetName={assetName as AssetName}
                                  refetchListener={refetchListener}
                                  hasErrorHandler={() => dispatchError({type: 'assetSummaryHasError'})}
                                  hasNoErrorHandler={() => dispatchError({type: 'assetSummaryHasNoError'})}
                    />
                    <AssetChart assetName={assetName as AssetName}
                                refetchListener={refetchListener}
                                hasErrorHandler={() => dispatchError({type: 'assetChartHasError'})}
                                hasNoErrorHandler={() => dispatchError({type: 'assetChartHasNoError'})}
                    />
                    <AssetMarkets assetName={assetName as AssetName}
                                  refetchListener={refetchListener}
                                  hasErrorHandler={() => dispatchError({type: 'assetMarketsHasError'})}
                                  hasNoErrorHandler={() => dispatchError({type: 'assetMarketsHasNoError'})}
                    />
                </AssetPriceWrapper>
            </AssetPriceContainer>
        </>
