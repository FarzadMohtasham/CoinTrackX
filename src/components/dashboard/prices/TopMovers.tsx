...
import TopMover from '@/Components/Dashboard/Prices/TopMover';
...
import { TopMover as TopMoverT } from '@/Libs/Typings/Components/TopMover.type';
...
export default function TopMovers(props: TopMoversProps): JSX.Element {
   ...
   useEffect((): void => {
      ...
      if (assets) {
         const sortedAssets: Asset[] = assets
            .sort((assetA: Asset, assetB: Asset) => {
               return (
                  Number(assetA.changePercent24Hr) -
                  Number(assetB.changePercent24Hr)
               );
            })
            .reverse()
            .slice(0, 3);

         const newTopMovers: TopMoverT[] = sortedAssets.map(
            (asset: Asset): TopMoverT => {
               return {
   ...
   return (
      <TopMoversContainer>
         <TopMoversHeaderWrapper>
            ...
            <Button
               variant={assetsDataLoadError ? 'primary' : 'gray'}
